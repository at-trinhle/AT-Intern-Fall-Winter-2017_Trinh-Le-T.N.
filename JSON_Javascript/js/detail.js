...
function showCart(data) {
  ...
  var listCart = '<table class="table"><thead><tr><th>Name</th><th>Price</th><th>Quantity</th><th>Total</th><th>Delete</th></tr></thead>';
  var id, name, price, count, total, sum = 0;

  for (let j of listProduct) {
    name = '<td>'+j.name+'</td>';
    price = '<td>'+j.price +'$</td>';
    count = '<td><input data-id="'+j.id+'" id="number-'+j.id+'" type="number" value="'+j.quantity+'" min="1" max="10" onclick="calculate(this)"></td>';
    total = '<td>'+j.price * j.quantity+'$</td>';
    deletebtn = '<td><button class="btn btn-danger" data-id="'+j.id+'">Delete</button></td>'
